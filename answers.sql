...
SELECT name, SUM(orders.num_cupcakes) AS sum
FROM cupcakes
LEFT JOIN orders
...
SELECT email FROM customers JOIN orders ON (orders.customer_id = customers.id) GROUP BY email HAVING SUM(orders.num_cupcakes) > 100;
...
SELECT fname, lname, email FROM customers JOIN orders ON (customers.id = orders.customer_id)
WHERE orders.processed = 'True'
AND cupcake_id = (SELECT cupcakes.id FROM cupcakes WHERE cupcakes.name = 'funfetti')
